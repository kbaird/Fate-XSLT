    ...
    <xsl:variable name="stressBoxAdditionsPure">
      <xsl:variable name="stressBoxAdditionsPureMax">
        <xsl:call-template name="calculate-pure-stress-by-rating">
          <xsl:with-param name="rating"><xsl:value-of select="$maxModeRating"/></xsl:with-param>
          <xsl:with-param name="stressType"><xsl:value-of select="$stressType"/></xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="stressBoxAdditionsPureMed">
        <xsl:call-template name="calculate-pure-stress-by-rating">
          <xsl:with-param name="rating"><xsl:value-of select="$medModeRating"/></xsl:with-param>
          <xsl:with-param name="stressType"><xsl:value-of select="$stressType"/></xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
      <xsl:variable name="stressBoxAdditionsPureMin">
        <xsl:call-template name="calculate-pure-stress-by-rating">
          <xsl:with-param name="rating"><xsl:value-of select="$minModeRating"/></xsl:with-param>
          <xsl:with-param name="stressType"><xsl:value-of select="$stressType"/></xsl:with-param>
        </xsl:call-template>
      </xsl:variable>
